...
public:
	...
	/* Gibt den Wert des Bruches dezimal zurück */
	double toDec() {
...
/* Operand +: Addiert zwei Brüche und gibt eine Referenz auf das Ergebnis als Bruch zurück. */
...
/* Subtrahiert zwei Brüche und gibt eine Referenz auf das Ergebnis als Bruch zurück. */
...
/* Multipliziert zwei Brüche und gibt eine Referenz auf das Ergebnis als Bruch zurück. */
cBruch& operator * (const cBruch &bruch1, const cBruch &bruch2) {
	// (a/b) * (c/d) = (a*c)/(b*d)
	static cBruch helper; 
...
/* Dividiert zwei Brüche und gibt eine Referenz auf das Ergebnis als Bruch zurück. */
cBruch& operator / (const cBruch &bruch1, const cBruch &bruch2) {
	// (a/b) : (c/d) = (a*d)/(b*c)
	static cBruch helper; 
...
int main() {
	...
	/* Geben Sie in einer Schleife die Werte aller 8 Brüche aus. */
	for each (cBruch bruch in Brueche) {
		bruch.ausgabe();
	}

	/* Addieren Sie die ersten beiden Brüche und geben Sie das Ergebnis aus mit der Funktion ausgabe() */
	b = Brueche[0] + Brueche[1];
	cout << "Addition der ersten beiden Br" << (char)129 <<"che: " << b << endl;
	cout << endl;

	/* Subtrahieren Sie die nächsten beiden Brüche und geben Sie das Ergebnis aus mit der Funktion ausgabe() */
	cout << "Subtraktion der n" << (char)132 << "chsten beiden Br" << (char)129 << "che: ";
	b = Brueche[2] - Brueche[3];
	b.ausgabe();
	cout << endl;

	/* Multiplizieren Sie die nächsten beiden Brüche und geben Sie das Ergebnis aus mit der Funktion ausgabe() */
	cout << "Multiplikation der n" << (char)132 << "chsten beiden Br" << (char)129 << "che: ";
	b = Brueche[4] * Brueche[5];
	b.ausgabe();
	cout << endl;

	/* Dividieren Sie die nächsten beiden Brüche und geben Sie das Ergebnis aus mit der Funktion ausgabe() */
	cout << "Division der n" << (char)132 << "chsten beiden Br" << (char)129 << "che: ";
	b = Brueche[6] / Brueche[7];
	b.ausgabe();
	cout << endl;

	/* Paarweiser Vergleich der Brüche */
	for (int i = 0; i < 8; i += 2) {
